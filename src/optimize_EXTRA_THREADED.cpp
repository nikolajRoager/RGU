...
std::array<int8_t,465> discrete_optimize_gains(std::function<size_t(std::array<int8_t,465>,size_t)> F, std::array<int8_t,465> x0,  size_t matches,size_t max_depth, std::default_random_engine& rng, int32_t random_repeats, std::array<int8_t,6> weights,std::vector<int> IDs,  bool verbose)
{
    ...
    auto compare=[matches,max_threads,&best_so_far, &best_so_far_err  ,&F_threaded,verbose](std::array<int8_t,465>* room_ptr,double* err, double* score,size_t* this_matches) -> void
    {
        ...
        *this_matches=0;
        ...
        do
        {
            F_threaded(&sub_wins,room_ptr,matches);
            wins+=sub_wins;
/*
...
*/
    ...
    while (!to_check_out.empty() && depth++<max_depth)
    {
        ...
        for (int i = 0; i < optimize_elements*2; i+=max_threads)
        {
            std::vector<double> err_array(max_threads);
            std::vector<double> score_array(max_threads);
            std::vector<size_t> matches_array(max_threads);


            std::vector<std::thread> myThreads(max_threads);

            if (verbose)
                std::cout<<"Launching threads"<<std::flush;




            for (uint64_t j=0; j<max_threads && i+j<neighbour_id.size(); j++)
            {

                if (verbose)
                    std::cout<<"Launching "<<j<<std::flush;

                myThreads[j]=std::thread(compare,&all_rooms[neighbour_id[i+j]].first,&err_array[j],&score_array[j],&matches_array[j]);


            }

            if (verbose)
                std::cout<<"... Reading threads "<<std::endl;

            for (uint64_t j=0; j<max_threads; j++)
            {
                myThreads[j].join();

                double score = score_array[j];
                double err = err_array[j];
                total_matches+=matches_array[j];

                all_rooms[neighbour_id[i+j]].second=score;

                gains[i+j]=all_rooms[neighbour_id[i+j]].second-this_score;

                if (verbose)
                    std::cout<<" Add neighbour "<<neighbour_id[i+j]<<"/"<<all_rooms.size()<<" with "<<score*100<<"%±"<<err*100<<'%'<<std::endl;




                to_check_out.insert({-score,&(all_rooms[neighbour_id[i+j]].first)});
                if (score>best_so_far)
                {
                    best_room=neighbour_id[i+j];
                    failed_steps=0;
                    if (verbose)
                ...
                max_gain = std::max(max_gain,gains[i+j]);
     ...
     //       std::cout<<"Gains, from current score "<<this_score<<std::endl;
        ...
        for (size_t i = current_top_of_rooms; i<all_rooms.size();i+=max_threads)
        {
            std::vector<double> err_array(max_threads);
            std::vector<double> score_array(max_threads);
            std::vector<size_t> matches_array(max_threads);
            ...
            for (uint64_t j=0; j<max_threads && i+j<all_rooms.size(); j++)
            {
                if (verbose)
                    std::cout<<"Launching "<<j<<std::flush;
                myThreads[j]=std::thread(compare,&all_rooms[i+j].first,&err_array[j],&score_array[j],&matches_array[j]);
            }
            ...
            for (uint64_t j=0; j<max_threads; j++)
            {
                ...
                all_rooms[i+j].second=score;

                //compare(&all_rooms[i].first,&err,&score,&this_matches);
 //               all_rooms[i].second=score;

                if (verbose)
                    std::cout<<" From random gain, add "<<(i+j)<<"/"<<all_rooms.size()<<" with "<<score*100<<"%±"<<err*100<<"% with "<<Picks[i+j-current_top_of_rooms]<<std::endl;
                to_check_out.insert({-score,&(all_rooms[i+j].first)});
